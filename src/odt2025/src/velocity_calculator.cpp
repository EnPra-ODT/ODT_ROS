...
struct AxisState {
    ...
    // 2-state: v (velocity) and b (accel bias)
    double v2 = 0.0;
    double b2 = 0.0;

    // 2x2 covariance for [v, b]
...
public:
    ImuKfLogger(ros::NodeHandle &nh) : nh_(nh)
    {
        ...
        // LPF for acceleration used by velocity propagation
        nh_.param("lpf_alpha", lpf_alpha_, 0.3);

        // Process noise (tuning) for 2-state KF
...
private:
    ...
    void cb(const std_msgs::Float64MultiArray::ConstPtr &msg){
        ...
        // If time goes backwards / stalls (wrap/reset), resync
        if (!(dt > 0.0)) {
    ...
    void processAxis(AxisState &S, double a_in, double deadband,
                     double F00, double F01, double F10, double F11, double dt)
    {
        // Light LPF on raw acceleration (used for velocity propagation)
        double a_lpf;
        if (!S.has_lpf_prev) {
            a_lpf = a_in;
            S.has_lpf_prev = true;
        } else {
            a_lpf = lpf_alpha_ * S.a_lpf_prev + (1.0 - lpf_alpha_) * a_in;
        }
        S.a_lpf_prev = a_lpf;

        const double u = a_lpf;

        // Stillness detection (ZUPT gate): use LPF accel vs deadband
        ...
        // ---- ZUPT update when still: measurement v = 0 ----
        if (still) {
            const double y   = -S.v2;               // innovation
            ...
            // covariance update (use temporaries)
