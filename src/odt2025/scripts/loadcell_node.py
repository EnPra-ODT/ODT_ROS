...
def main():
    ...
    pub_contact_pair = rospy.Publisher(
     ...
     queue_size=10
        ...
        # 各ロードセルの接地状態を配列で送る
    # [ch1, ch2] -> [1.0, 0.0] など
    pub_contact_array = rospy.Publisher(
        '/foot_contact_array',
        Float64MultiArray,
        queue_size=10
    )
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            if len(vals) != 8:
                rospy.logwarn(f"Invalid line: {line}")
                continue
           ...
           # 各ロードセルの接地状態を配列で送信
            ...
            rospy.loginfo(f"CH1={ch1_N:.2f} N CH2={ch2_N:.1f}N CH3={ch3_N:.1f}N CH4={ch4_N:.1f} CH5={ch5_N:.1f}N CH6={ch6_N:.1f}N CH7={ch7_N:.1f}N CH8={ch8_N:.1f} pair={pair_array.data}")
