...
#include <xmlrpcpp/XmlRpcValue.h>  // <-- FIX: needed for XmlRpc::XmlRpcValue
...
//=============== Control Word Definitions (0x6040) ===============//
...
public:
  ...
  bool recvFrame(struct can_frame* out, int timeout_ms) {
    std::lock_guard<std::mutex> lk(mtx_);
    return recvFrameUnlocked(out, timeout_ms);
  }

  // <-- FIX: drain RX to prevent SDO ack backlog from starving reads
  void drainRx(int max_frames = 2000) {
    std::lock_guard<std::mutex> lk(mtx_);
    for (int i = 0; i < max_frames; i++) {
      struct can_frame f;
      if (!recvFrameUnlocked(&f, 0)) break;
    }
  }
...
private:
  ...
  bool recvFrameUnlocked(struct can_frame* out, int timeout_ms) {
    fd_set rfds;
    struct timeval tv;
    FD_ZERO(&rfds);
    FD_SET(sock_, &rfds);
  ...
  void open(const std::string& ifname) {
    ...
    // Optional: make sure we DON'T receive our own transmitted frames
    int recv_own = 0;
...
public:
  ...
  bool readI32(uint16_t index, uint8_t sub, int32_t* out_val, int window_ms) {
    ...
    // <-- FIX: clear old SDO responses/acks so this read doesn't starve
    bus_.drainRx();
...
public:
  KincoMotor(CanSocketBus& bus, int node_id, MotorConfig cfg) : cfg_(cfg), sdo_(bus, node_id) {
    cfg_.scale_num = 512.0 * (double)cfg_.encoder_res;
  }
  ...
  void setTargetRpm(double rpm) {
    ...
    if (rpm < -cfg_.max_rpm) rpm = -cfg_.max_rpm;
    sdo_.writeI32(0x60FF, 0x00, rpmToDriveUnits(rpm));
  }
...
static std::vector<int> getNodeIdsParam(ros::NodeHandle& pnh) {
  ...
  if (!pnh.getParam("node_ids", v)) { ids.push_back(1); return ids; }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    motors.reserve(node_ids.size()); // keep pointers stable

    for (int id : node_ids) {
      motors.emplace_back();                 // <-- FIX: create element in vector first
      MotorCtx* ctx = &motors.back();        // stable pointer (reserve prevents reallocation)
      ...
      // <-- FIX: capture ctx POINTER, not reference to a local that gets moved/destroyed
    ...
    ros::Timer poll_timer = nh.createTimer(
      ...
      [&motors](const ros::TimerEvent&) {
        for (auto& ctx : motors) {
          double rpm = 0.0;
          if (!ctx.motor->readActualRpm(&rpm, 30)) continue; // slightly larger window
